# PowerShell script that downloads the latest version of Modune and runs the installer
...
function Get-FileFromWeb {
    ...
    try {
        ...
        do {
            ...
            $total += $count
    if ($fullSize -gt 0) { Show-Progress -TotalValue $fullSize -CurrentValue $total -ProgressText " Downloading Modune" }
        } while ($count -gt 0)
    }
    finally {
        $reader.Close()
...
$installerPath = "C:\ProgramData\Tweaker\Unattend\TweakerInstaller.exe"
$downloadUrl = $env:MODUNE_RELEASE_DOWNLOAD_URL
if ([string]::IsNullOrWhiteSpace($downloadUrl)) { throw "MODUNE_RELEASE_DOWNLOAD_URL is not configured." }
...
Write-Host "          Modune Installer" -ForegroundColor Cyan
    Write-Host "========================================" -ForegroundColor Cyan
    Write-Host ""
    ...
    Write-Host ""
Write-Host "Downloading Modune ($installType)..." -ForegroundColor Cyan
    ...
    Write-Host "Installing Modune..." -ForegroundColor Cyan
    ...
    Write-Host "Launching Modune..." -ForegroundColor Cyan
        ...
        Write-Host ""
    Write-Host "Extracting Modune Portable to Desktop..." -ForegroundColor Cyan
