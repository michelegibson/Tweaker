...
#define MyAppName "Tweaker"
#define MyAppDisplayName "Modune"
#define MyAppVersion ""
#define MyAppPublisher "Marco du Plessis"
#define MyAppURL "https://www.tweaker.net/"
#define MyAppExeName "Tweaker.exe"
#define MyAppAssocName MyAppName + " File"
#define MyAppAssocDisplayName MyAppDisplayName + " Configuration"
#define MyAppAssocExt ".tweaker"
...
AppVerName=Modune
SetupMutex=TweakerSetupMutex,Global
...
LicenseFile=C:\Tweaker\LICENSE.txt
; Uncomment the following line to run in non administrative install mode (install for current user only).
;PrivilegesRequired=lowest
PrivilegesRequiredOverridesAllowed=dialog
OutputDir=C:\Tweaker\installer-output
OutputBaseFilename=Tweaker.Installer
SetupIconFile=C:\Tweaker\src\Tweaker.UI\Assets\AppIcons\modune-mark.ico
...
begin
  ...
  if CheckForMutexes('TweakerSetupMutex') then
  begin
    if MsgBox('Modune is currently running. Do you want to close it before uninstalling?', 
...
begin
  ...
  // Append \Tweaker if the path doesn't already end with it.
  //
  ...
  //     ({autopf}\Tweaker for regular, {userdesktop}\Tweaker for portable),
  //     silently relocating the update.
  //   - Tweaker.ps1 first-time install (silent, no /DIR=): keep the sync. The
  //     PS1 relies on it to redirect a portable install from DefaultDirName
  //     ({autopf}\Tweaker) to {userdesktop}\Tweaker.
...
;    self-contained build. Tweaker calls no ML/AI APIs, so they are dead weight -
...
Root: HKA; Subkey: "Software\Classes\{#MyAppAssocKey}"; ValueType: string; ValueName: ""; ValueData: "{#MyAppAssocDisplayName}"; Flags: uninsdeletekey; Tasks: regularinstall
...
Name: "{autoprograms}\{#MyAppDisplayName}"; Filename: "{app}\{#MyAppExeName}"; Tasks: regularinstall\startmenuicon
; Create desktop shortcut only if selected in tasks
Name: "{autodesktop}\{#MyAppDisplayName}"; Filename: "{app}\{#MyAppExeName}"; Tasks: regularinstall\desktopicon

[Run]
; Launch application after installation
Filename: "{app}\{#MyAppExeName}"; Description: "{cm:LaunchProgram,{#StringChange(MyAppDisplayName, '&', '&&')}}"; Flags: shellexec nowait postinstall skipifsilent
