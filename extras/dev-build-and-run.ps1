...
#
# Bumps the Tweaker.UI csproj Version / FileVersion / AssemblyVersion to
...
# when TWEAKER_LOCAL_BUILD_ROOT is in effect — a populated in-tree obj/
...
$csprojPath   = Join-Path $repoRoot 'src\Tweaker.UI\Tweaker.UI.csproj'
$msbuild      = Join-Path ${env:ProgramFiles} 'Microsoft Visual Studio\18\Community\MSBuild\Current\Bin\MSBuild.exe'
$buildLogDir  = Join-Path $env:LOCALAPPDATA 'Tweaker-dev'
...
if ($repoIsRemote) {
    ...
    $exePath = Join-Path $localBuildRoot "Tweaker.UI\bin\x64\Debug\$tfm\win-x64\Tweaker.exe"
}
else {
    $exePath = Join-Path $repoRoot "src\Tweaker.UI\bin\x64\Debug\$tfm\win-x64\Tweaker.exe"
}
...
# (via TWEAKER_LOCAL_BUILD_ROOT + src\Directory.Build.props). The in-tree
...
# local path via the TWEAKER_LOCAL_BUILD_ROOT env var, which
...
& $msbuild `
    'src\Tweaker.UI\Tweaker.UI.csproj' `
