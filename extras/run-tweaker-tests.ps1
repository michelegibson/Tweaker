# run-tweaker-tests.ps1
# Runs all Tweaker test suites and reports results.
#
# SYNOPSIS:
...
# .\run-tweaker-tests.ps1 -SkipUITests
#
# # Run only integration tests
# .\run-tweaker-tests.ps1 -IntegrationOnly
param (
...
Write-Host "  Tweaker Test Runner" -ForegroundColor Cyan
Write-Host ("=" * 60) -ForegroundColor Cyan
...
if (-not $IntegrationOnly) {
    ...
    Run-TestProject `
        ...
        -ProjectPath "$solutionDir\tests\Tweaker.Core.Tests\Tweaker.Core.Tests.csproj"

    # Unit Tests - Infrastructure
    Run-TestProject `
        -Name "Infrastructure Unit Tests" `
        -ProjectPath "$solutionDir\tests\Tweaker.Infrastructure.Tests\Tweaker.Infrastructure.Tests.csproj"

    # Unit Tests - UI (requires Visual Studio / WinUI SDK, built with MSBuild)
    if (-not $SkipUITests) {
        $uiProjectPath = "$solutionDir\tests\Tweaker.UI.Tests\Tweaker.UI.Tests.csproj"
        ...
        if (-not $msbuildPath -or -not (Test-Path $msbuildPath)) {
            ...
            if ($LASTEXITCODE -ne 0) {
                ...
                $uiTestDll = "$solutionDir\tests\Tweaker.UI.Tests\bin\x64\Debug\net10.0-windows10.0.19041.0\Tweaker.UI.Tests.dll"
...
Run-TestProject `
    ...
    -ProjectPath "$solutionDir\tests\Tweaker.IntegrationTests\Tweaker.IntegrationTests.csproj"
