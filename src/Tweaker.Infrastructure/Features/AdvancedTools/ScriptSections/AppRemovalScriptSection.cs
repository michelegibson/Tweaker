...
using Tweaker.Core.Features.SoftwareApps.Utilities;

namespace Tweaker.Infrastructure.Features.AdvancedTools.ScriptSections;

/// <summary>
/// Handles scripts directory setup, app removal script embedding, and the Tweaker installer script.
/// </summary>
internal class AppRemovalScriptSection
{
    ...
    public async Task AppendBloatRemovalScriptAsync(StringBuilder sb, IReadOnlyList<ConfigurationItem> selectedApps, string indent = "")
    {
        ...
        sb.AppendLine();
        sb.AppendLine($"{indent}            Register-ScheduledTask -TaskName $script.Name -TaskPath \"\\Tweaker\" -Action $action -Trigger $trigger -Settings $settings -Principal $principal -Force | Out-Null");
    ...
    public void AppendTweakerInstallerScriptContent(StringBuilder sb, string indent = "")
    {
        sb.AppendLine($"{indent}# Create desktop shortcut for Modune installer");
        sb.AppendLine($"{indent}try {{");
        sb.AppendLine($"{indent}    $shortcutPath = \"C:\\Users\\Default\\Desktop\\Install Modune.lnk\"");
        ...
        sb.AppendLine($"{indent}    $shortcut.Arguments = \"-ExecutionPolicy Bypass -NoProfile -Command `\"irm 'https://get.tweaker.net' | iex`\"\"");
        ...
        sb.AppendLine($"{indent}    $shortcut.Description = \"Download and install Modune\"");
