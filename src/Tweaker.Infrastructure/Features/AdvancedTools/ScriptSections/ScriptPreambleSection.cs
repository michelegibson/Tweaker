...
internal static class ScriptPreambleSection
{
    public static void AppendHeader(StringBuilder sb)
    {
        sb.AppendLine($@"<#
.SYNOPSIS
    Modune Windows 10/11 Customization and Optimization Script
.DESCRIPTION
...
.NOTES
    ...
    Logs all activities to C:\ProgramData\Tweaker\Unattend\Logs\Tweakerments.txt
.PARAMETER UserCustomizations
    ...
    To re-apply, delete: HKCU\Software\Tweaker\UserCustomizationsApplied
.EXAMPLE
    .\Tweakerments.ps1
    Runs in normal mode - applies all system-wide settings (HKLM) but skips user settings (HKCU)
.EXAMPLE
    .\Tweakerments.ps1 -UserCustomizations
...
$LogPath = 'C:\ProgramData\Tweaker\Unattend\Logs\Tweakerments.txt'
...
# Initialize log file
Write-Log ""================================================================================="" ""INFO""
Write-Log ""Modune Windows Optimization & Customization Script Started"" ""INFO""
...
function Start-ProcessAsUser {
    ...
    if (-not ([System.Management.Automation.PSTypeName]'Tweaker.TL'.Type)) {
...
'@ -Name TL -Namespace Tweaker -ErrorAction Stop
    }

    $T = [Tweaker.TL]
    $tok = $dup = $envBlock = [IntPtr]::Zero
    $pi = New-Object Tweaker.TL+PI
    $launched = $false
...
");
    ...
    public static void AppendCompletionBlock(StringBuilder sb)
    {
        ...
        sb.AppendLine("Write-Log \"Modune Windows Optimization & Customization Script Completed\" \"SUCCESS\"");
        sb.AppendLine("Write-Log \"================================================================================\" \"INFO\"");
    }
}
