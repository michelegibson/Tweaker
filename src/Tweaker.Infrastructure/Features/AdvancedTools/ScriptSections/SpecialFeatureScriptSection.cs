...
internal static class SpecialFeatureScriptSection
{
    public static void AppendUserCustomizationsScheduledTask(StringBuilder sb, string indent)
    {
        ...
        sb.AppendLine($"{indent}    Register-ScheduledTask -TaskName \"TweakerUserCustomizations\" -TaskPath \"\\Tweaker\" -Action $action -Trigger $trigger -Settings $settings -Principal $principal -Force | Out-Null");
        sb.AppendLine($"{indent}    Write-Log \"Registered scheduled task: TweakerUserCustomizations\" \"SUCCESS\"");
