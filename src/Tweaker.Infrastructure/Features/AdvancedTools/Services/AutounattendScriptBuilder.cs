...
public class AutounattendScriptBuilder
{
    ...
    public async Task<string> BuildTweakermentsScriptAsync(
        ...
        IReadOnlyDictionary<string, IEnumerable<SettingDefinition>> allSettings)
    {
        ...
        _appRemovalSection.AppendTweakerInstallerScriptContent(sb, "    ");
        ...
        try
        {
            ...
            _logService.Log(LogLevel.Info, "Tweakerments.ps1 script passed PowerShell syntax validation");
        }
        catch (Exception ex)
        {
            _logService.Log(LogLevel.Error, $"Tweakerments.ps1 script failed PowerShell syntax validation: {ex.Message}");
            throw;
        }
    ...
    private static void AppendUserDetectionBridge(StringBuilder sb)
    {
        ...
        sb.AppendLine("        $markerPath = \"HKU:\\$targetUserSID\\Software\\Tweaker\"");
        ...
        sb.AppendLine("            Write-Log \"To re-apply, delete: HKCU\\Software\\Tweaker\\$markerName\" \"INFO\"");
        ...
        sb.AppendLine();
        sb.AppendLine("        $markerPath = \"HKCU:\\Software\\Tweaker\"");
        sb.AppendLine("        $markerName = \"UserCustomizationsApplied\"");
        sb.AppendLine("        $alreadyApplied = $false");
        sb.AppendLine();
        sb.AppendLine("        try {");
