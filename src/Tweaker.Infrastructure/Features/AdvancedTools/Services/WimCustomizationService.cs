...
namespace Tweaker.Infrastructure.Features.AdvancedTools.Services;
...
public class WimCustomizationService : IWimCustomizationService
{
    ...
    private static readonly string? UnattendedXmlUrl = Environment.GetEnvironmentVariable("MODUNE_UNATTEND_XML_URL");
        ...
        CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            if (string.IsNullOrEmpty(driverSourcePath))
            {
                ...
                var tempDriverPath = _fileSystemService.CombinePath(_fileSystemService.GetTempPath(), $"TweakerDrivers_{Guid.NewGuid()}");
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            progress?.Report(new TaskProgressDetail
            {
                ...
                TerminalOutput = UnattendedXmlUrl ?? "Remote XML endpoint is not configured"
            });

            if (string.IsNullOrWhiteSpace(UnattendedXmlUrl))
                throw new InvalidOperationException("No remote unattended XML endpoint is configured.");

            var xmlContent = await _httpClient.GetStringAsync(UnattendedXmlUrl, cancellationToken).ConfigureAwait(false);
