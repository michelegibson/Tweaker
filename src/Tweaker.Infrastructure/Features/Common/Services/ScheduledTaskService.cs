...
public class ScheduledTaskService(ILogService logService, IFileSystemService fileSystemService) : IScheduledTaskService
{
    ...
    public async Task<OperationResult> UnregisterScheduledTaskAsync(string taskName)
    {
        return await Task.Run(() =>
        {
            ...
            try
            {
                ...
                folder = GetTweakerFolder(taskService);
    ...
    public async Task<OperationResult> RunScheduledTaskAsync(string taskName)
    {
        return await Task.Run(() =>
        {
            ...
            try
            {
                ...
                if (folder == null)
                {
                    logService.LogError($"Modune task folder not found when trying to run: {taskName}");
                    return OperationResult.Failed("Modune task folder not found");
                }
    ...
    private async Task<OperationResult> RegisterTaskInternal(string taskName, string? scriptPath, string? username, TaskTriggerType triggerType, string? command = null)
    {
        ...
        {
            taskService = CreateTaskService();
            folder = GetOrCreateTweakerFolder(taskService);
    ...
    private dynamic GetOrCreateTweakerFolder(dynamic taskService)
    {
        dynamic rootFolder = taskService.GetFolder("\\");
        try
        {
            return rootFolder.GetFolder("Tweaker");
        }
        catch (Exception ex)
        {
            logService.Log(Core.Features.Common.Enums.LogLevel.Debug, $"[ScheduledTaskService] Tweaker folder doesn't exist, creating: {ex.Message}");
            return rootFolder.CreateFolder("Tweaker");
        }
        finally
        {
    ...
    private dynamic? GetTweakerFolder(dynamic taskService)
    {
        ...
        {
            logService.Log(Core.Features.Common.Enums.LogLevel.Debug, $"[ScheduledTaskService] Tweaker folder not found: {ex.Message}");
            return null;
        }
        finally
        {
