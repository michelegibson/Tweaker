...
{
    // Dev-only override: when TWEAKER_LOCAL_INSTALLER points at an existing
    // file, the in-app updater skips the remote release lookup entirely and uses
    ...
    private const string LocalInstallerEnvVar = "TWEAKER_LOCAL_INSTALLER";

    private readonly ILogService _logService;
    private readonly IProcessExecutor _processExecutor;
    private readonly IFileSystemService _fileSystemService;
    private readonly HttpClient _httpClient;
    private readonly string? _latestReleaseApiUrl = Environment.GetEnvironmentVariable("MODUNE_UPDATE_API_URL");
    private readonly string? _latestReleaseDownloadUrl = Environment.GetEnvironmentVariable("MODUNE_UPDATE_DOWNLOAD_URL");
    private readonly string _userAgent = "Tweaker-Update-Checker";
    ...
    public async Task<VersionInfo> CheckForUpdateAsync(CancellationToken cancellationToken = default)
    {
        ...
        if (TryGetLocalInstallerOverride() is { } localPath)
        {
            _logService.Log(LogLevel.Warning,
                $"[DEV MODE] {LocalInstallerEnvVar} set to {localPath}. Skipping remote release lookup and reporting a synthetic update.");
        ...
        if (string.IsNullOrWhiteSpace(_latestReleaseApiUrl))
        {
            _logService.Log(LogLevel.Info, "Online update checks are disabled because no update endpoint is configured.");
        ...
        for (int attempt = 1; attempt <= maxRetries; attempt++)
        {
            try
            {
                ...
                // Get the latest release information from configured release API
    ...
    public async Task DownloadAndInstallUpdateAsync(CancellationToken cancellationToken = default)
    {
        ...
        if (string.IsNullOrWhiteSpace(_latestReleaseDownloadUrl))
            throw new InvalidOperationException("No update download endpoint is configured.");

        _logService.Log(LogLevel.Info, "Downloading update...");

        // Create a temporary file to download the installer
        string tempPath = _fileSystemService.CombinePath(_fileSystemService.GetTempPath(), "Tweaker.Installer.exe");
    ...
    public void LaunchInstallerAndRestart()
    {
        ...
        string appExePath = _fileSystemService.CombinePath(appDir, "Tweaker.exe");
    ...
    /// <summary>
    /// Returns the local installer path if TWEAKER_LOCAL_INSTALLER is set and
    ...
    /// can resolve {app} to C:\Program Files\Tweaker for regular installs at a
    /// custom path, or to ~\Desktop\Tweaker for portable installs, silently
    /// relocating the install. The installer script (Tweaker.Installer.iss)
