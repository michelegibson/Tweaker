...
{
    private const string CacheSubDir = @"Tweaker\IconCache";

    // Cache filenames are <def.Id>.<short-hash>.png. The id makes them readable
    // when poking around %ProgramData%\Tweaker\IconCache; the 8-char hash of
    ...
    // manually wipe %ProgramData%\Tweaker\IconCache so cached files re-extract.
    ...
    /// <summary>Production constructor — uses %ProgramData%\Tweaker\IconCache.</summary>
    ...
    // logged-in user looking at Tweaker's catalog should see. ProgramData
    // (%CommonApplicationData%) puts the cache alongside Tweaker's other
    // shared state (e.g. C:\ProgramData\Tweaker\Logs) and means a single
        ...
        bool allowRawFallback = true)
    {
        ...
        // %ProgramData%\Tweaker\Logs instead of silently dropping the
