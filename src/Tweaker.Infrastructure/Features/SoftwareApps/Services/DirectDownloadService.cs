...
public class DirectDownloadService : IDirectDownloadService
{
    ...
    private static readonly string UserAgent = "Tweaker-Download-Manager";
    ...
    {
        var tempPath = _fileSystemService.CombinePath(_fileSystemService.GetTempPath(), $"Tweaker_{item.Id}_{Guid.NewGuid():N}");
        ...
        CancellationToken cancellationToken)
    {
        try
        {
            var logPath = _fileSystemService.CombinePath(_fileSystemService.GetTempPath(), $"Tweaker_MSI_{Guid.NewGuid():N}.log");
            ...
            if (exitCode == 1612)
            {
                ...
                logPath = _fileSystemService.CombinePath(_fileSystemService.GetTempPath(), $"Tweaker_MSI_{Guid.NewGuid():N}.log");
