...
namespace Tweaker.Infrastructure.Features.SoftwareApps.Services.WinGet.Utilities;
...
public class WinGetInstaller
{
    ...
    private static readonly string? DownloadBaseUrl = Environment.GetEnvironmentVariable("MODUNE_WINGET_DOWNLOAD_BASE_URL");
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        // Option 2b: Download from a configured endpoint and provision
        if (string.IsNullOrWhiteSpace(DownloadBaseUrl))
            return (false, "WinGet download endpoint is not configured");

        _logService?.LogInformation("No existing App Installer package found, downloading from configured endpoint...");
        ...
        try
        {
            ...
            await Task.WhenAll(
                DownloadFileAsync($"{DownloadBaseUrl}/{DependenciesFileName}", dependenciesPath, "Dependencies", false, 0, 0, cancellationToken),
                DownloadFileAsync($"{DownloadBaseUrl}/{InstallerFileName}", installerPath, "WinGet Installer", true, 0, 45, cancellationToken),
                DownloadFileAsync($"{DownloadBaseUrl}/{LicenseFileName}", licensePath, "License", false, 0, 0, cancellationToken)
        ...
        CancellationToken cancellationToken)
    {
        try
        {
            ...
            // A bare -1 (0xFFFFFFFF) is Tweaker's own kill, not a winget verdict (see issue #675)
