...
namespace Tweaker.UI.Features.AdvancedTools.Services;
...
public class AutounattendXmlGeneratorService : IAutounattendXmlGeneratorService
{
    ...
    private async Task<string> RenderConfigToXmlAsync(UnifiedConfigurationFile config, string outputPath)
    {
        ...
        var scriptContent = await _scriptBuilder.BuildTweakermentsScriptAsync(config, allSettings);
    ...
    private string LoadEmbeddedTemplate()
    {
        ...
        var resourceName = "Tweaker.UI.Resources.AdvancedTools.autounattend-template.xml";
