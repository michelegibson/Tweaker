...
namespace Tweaker.UI.Features.Common.Controls;
...
{
    // Spacious studio navigation with a compact icon-only rail.
    private const double ExpandedWidth = 240;
    private const double CompactWidth = 64;
    ...
    /// </summary>
    public Thickness NavPanelPadding => IsPaneOpen ? new Thickness(12, 0, 12, 0) : new Thickness(8, 0, 8, 0);

    public Visibility ExpandedContentVisibility => IsPaneOpen ? Visibility.Visible : Visibility.Collapsed;
        ...
        foreach (var item in MoreMenuFlyout.Items)
        {
            if (item is MenuFlyoutItem menuItem)
            {
                ...
                menuItem.Text = tag switch
                {
                    ...
                    "OpenLogs" => _moreMenuViewModel.MenuTweakerLogs,
                    "OpenChangeHistory" => _moreMenuViewModel.MenuChangeHistory,
                    "OpenScripts" => _moreMenuViewModel.MenuTweakerScripts,
                    "CloseApp" => _moreMenuViewModel.MenuCloseTweaker,
                    _ => menuItem.Text
                };
    ...
    private static void OnIsPaneOpenChanged(DependencyObject d, DependencyPropertyChangedEventArgs e)
    {
        if (d is NavSidebar sidebar)
        {
            ...
            sidebar.NotifyPropertyChanged(nameof(ExpandedContentVisibility));
        }
    }
