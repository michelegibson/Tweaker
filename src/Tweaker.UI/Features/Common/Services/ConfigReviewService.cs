...
public class ConfigReviewService : IConfigReviewService, IConfigReviewModeService, IConfigReviewDiffService, IConfigReviewBadgeService, IApplicationModeService, IDisposable
{
    ...
    public TweakerMode CurrentMode { get; private set; } = TweakerMode.Normal;
    ...
    public bool IsInReviewMode => CurrentMode == TweakerMode.ConfigReview;
    ...
    public async Task EnterReviewModeAsync(UnifiedConfigurationFile config, bool isWindowsDefaults = false)
    {
        ...
        CurrentMode = TweakerMode.ConfigReview;
    ...
    public void ExitReviewMode() => SetMode(TweakerMode.Normal);

    public void EnterBuilderMode(BuilderTarget target) => SetMode(TweakerMode.Builder, target);
    ...
    /// </summary>
    private void SetMode(TweakerMode target, BuilderTarget builderTarget = BuilderTarget.Config)
    {
        LeaveCurrentMode();

        if (target == TweakerMode.Builder)
        {
            CurrentBuilderTarget = builderTarget;
        }

        CurrentMode = target;
        _logService.Log(LogLevel.Info, target == TweakerMode.Builder
    ...
    private void LeaveCurrentMode()
    {
        switch (CurrentMode)
        {
            case TweakerMode.ConfigReview:
                ...
                CurrentMode = TweakerMode.Normal;
                ReviewModeChanged?.Invoke(this, EventArgs.Empty);
                BadgeStateChanged?.Invoke(this, EventArgs.Empty);
                break;
    ...
    {
        if (CurrentMode != TweakerMode.Builder || CurrentBuilderTarget == target)
        {
            return;
        }
    ...
    {
        if (CurrentMode == TweakerMode.Normal)
        {
            return;
        }

        SetMode(TweakerMode.Normal);
    }
        ...
        switch (settingDef.InputType)
        {
            ...
            case InputType.Selection:
            {
                ...
                if (configItem.PowerPlanGuid != null)
                {
                    ...
                    // This handles branded plans created with a different runtime GUID,
                    // including configurations exported before the Modune rename.
    ...
    private static PredefinedPowerPlan? ResolveToPredefinedPlan(string? guid, string? name)
    {
        ...
        {
            // The current and legacy branded aliases are exact matches. Avoid broad
            // substring matching so unrelated plans cannot be treated as equivalent.
            if (PowerPlanDefinitions.IsModunePowerPlanName(name))
            {
