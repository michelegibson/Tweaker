...
namespace Tweaker.UI.Features.Common.Views;
...
public sealed partial class ConfigImportOverlayWindow : Window
{
    ...
    public ConfigImportOverlayWindow(string statusText, string? detailText = null)
    {
        ...
        AutomationProperties.SetName(OverlayProgressRing, statusText);

        // This is a separate Window, so inherit the user's in-app theme explicitly.
        try
        {
            var themeService = App.Services.GetService<IThemeService>();
            RootGrid.RequestedTheme = themeService?.GetEffectiveTheme() ?? ElementTheme.Default;
        }
        catch
        {
        ...
        OverlayLogo.Source = new BitmapImage(
                new Uri("ms-appx:///Assets/AppIcons/modune-mark.png"));

        try
        {
            var localizationService = App.Services.GetService<ILocalizationService>();
            OverlayTitleText.Text = localizationService?.GetString("App_Title") ?? "Modune";
            OverlayTaglineText.Text = localizationService?.GetString("App_Tagline") ?? "";
        }
        catch
        {
            OverlayTitleText.Text = "Modune";
    ...
    public void UpdateStatus(string status, string? detail = null)
    {
        DispatcherQueue.TryEnqueue(() =>
        {
            ...
            AutomationProperties.SetName(OverlayProgressRing, status);
