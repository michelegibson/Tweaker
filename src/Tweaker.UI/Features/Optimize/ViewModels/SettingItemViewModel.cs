...
public partial class SettingItemViewModel : BaseViewModel
{
    ...
    private int? FindOptionIndex(Func<Tweaker.Core.Features.Common.Models.ComboBoxOption, bool> predicate)
    {
    ...
    private bool IsBuilderMode => _applicationModeService?.CurrentMode == TweakerMode.Builder;
    ...
    private static int FindIndexForPowerCfgValue(IReadOnlyList<Tweaker.Core.Features.Common.Models.ComboBoxOption> options, int targetValue)
    {
    ...
    private (string Label, string Tooltip) ResolvePillStrings(SettingBadgeKind kind, SettingBadgeMode mode = SettingBadgeMode.None)
    {
        var (baseLabel, tooltip) = kind switch
        {
            ...
            _localizationService?.GetString("InfoBadge_Recommended_Tooltip") ?? "Modune's recommended value"),
