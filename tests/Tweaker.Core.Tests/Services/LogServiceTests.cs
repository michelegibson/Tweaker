...
namespace Tweaker.Core.Tests.Services;
...
public class LogServiceTests
{
    ...
    [Fact]
    public void GetLogPath_ContainsTweakerDirectory()
    {
        using var service = new LogService();

        var path = service.GetLogPath();

        path.Should().Contain("Tweaker");
    ...
    public void Log_WithDifferentLevels_DoesNotThrow()
    {
        ...
        var action = () =>
        {
            service.Log(Tweaker.Core.Features.Common.Enums.LogLevel.Info, "info message");
            service.Log(Tweaker.Core.Features.Common.Enums.LogLevel.Warning, "warning message");
            service.Log(Tweaker.Core.Features.Common.Enums.LogLevel.Error, "error message");
            service.Log(Tweaker.Core.Features.Common.Enums.LogLevel.Debug, "debug message");
            service.Log(Tweaker.Core.Features.Common.Enums.LogLevel.Success, "success message");
        };
    ...
    public void Log_WithException_DoesNotThrow()
    {
        ...
        var action = () => service.Log(
            Tweaker.Core.Features.Common.Enums.LogLevel.Error, "error", ex);
    ...
    public void StartLog_WithSystemInfoProvider_WritesAllDiagnosticFields()
    {
        ...
        logContent.Should().Contain("Modune 26.02.20 Log Started");
    ...
    public void StartLog_WithoutSystemInfoProvider_WritesFallbackMessage()
    {
        ...
        var logContent = File.ReadAllText(logPath);
        logContent.Should().Contain("Modune Log Started");
    ...
    {
        var tempDir = Path.Combine(Path.GetTempPath(), $"TweakerLogTest_{Guid.NewGuid():N}");
        Directory.CreateDirectory(tempDir);
        try
        {
            // Create an "old" log file with a creation time well in the past
            var oldFile = Path.Combine(tempDir, "Tweaker_Log_20200101_000000.log");
            File.WriteAllText(oldFile, "old");
            File.SetCreationTimeUtc(oldFile, DateTime.UtcNow.AddDays(-60));

            // Create a "recent" log file
            var recentFile = Path.Combine(tempDir, "Tweaker_Log_20260226_120000.log");
    ...
    public void CleanupOldLogs_CapsToMaxFiles()
    {
        ...
        try
        {
            ...
            for (int i = 0; i < 5; i++)
            {
                var file = Path.Combine(tempDir, $"Tweaker_Log_20260226_{i:D6}.log");
            ...
            var remaining = Directory.GetFiles(tempDir, "Tweaker_Log_*.log");
