...
using Tweaker.Infrastructure.Features.AdvancedTools.ScriptSections;
using Xunit;
...
public class AppRemovalScriptSectionTests
{
    ...
    public void AppendScriptsDirectorySetup_ContainsScriptsDirectory()
    {
        ...
        output.Should().Contain("C:\\ProgramData\\Tweaker\\Scripts");
    }
    ...
    // ---------------------------------------------------------------
    // AppendTweakerInstallerScriptContent
    // ---------------------------------------------------------------

    [Fact]
    public void AppendTweakerInstallerScriptContent_ContainsInstallerScript()
    {
        var sb = new StringBuilder();

        _sut.AppendTweakerInstallerScriptContent(sb);

        var output = sb.ToString();
        output.Should().Contain("Install Modune.lnk");
        output.Should().Contain("CreateShortcut");
    }

    [Fact]
    public void AppendTweakerInstallerScriptContent_ContainsDownloadUrl()
    {
        var sb = new StringBuilder();

        _sut.AppendTweakerInstallerScriptContent(sb);

        sb.ToString().Should().Contain("get.tweaker.net");
    }

    [Fact]
    public void AppendTweakerInstallerScriptContent_ContainsDesktopShortcutCreation()
    {
        var sb = new StringBuilder();
    ...
    [Fact]
    public void AppendTweakerInstallerScriptContent_UsesProvidedIndent()
    {
        var sb = new StringBuilder();

        _sut.AppendTweakerInstallerScriptContent(sb, "        ");

        sb.ToString().Should().Contain("        # Create desktop shortcut for Modune installer");
    }

    [Fact]
    public void AppendTweakerInstallerScriptContent_ContainsErrorHandling()
    {
        var sb = new StringBuilder();
