...
public class ChangeHistoryServiceTests
{
    ...
    public ChangeHistoryServiceTests()
    {
        ...
        _mockLocalization.Setup(l => l.GetString("ChangeHistory_FileHeader"))
            .Returns("Changes made by Modune are listed below (newest at the bottom).");
    ...
    {
        using (_service.BeginBatch("Config import (my-config.tweaker)"))
        {
            _appended.Should().BeEmpty("header is lazy — nothing written until the first entry");
            _service.LogSettingChange("Telemetry", "Privacy", "Enabled", "Disabled");
        }

        _appended.Should().Contain("] Config import (my-config.tweaker):\r\n");
    ...
    {
        using (_service.BeginBatch("Config import (empty.tweaker)")) { }
    ...
    public void FirstWrite_FileMissing_WritesLocalizedHeaderFirst()
    {
        ...
        firstAppend.Should().Contain("Changes made by Modune");
    ...
    [Fact]
    public void GetFilePath_ReturnsProgramDataTweakerPath()
    {
