...
public class PowerPlanComboBoxServiceTests
{
    ...
    [Fact]
    public async Task GetPowerPlanOptionsAsync_CanonicalAndLegacyAliases_AppearAsOneLogicalOption()
    {
        var legacyRuntimeGuid = "11111111-2222-3333-4444-555555555555";
        var systemPlans = new List<PowerPlan>
        {
            new()
            {
            ...
            new()
            {
                Name = PowerPlanDefinitions.LegacyTweakerPowerPlanName,
                Guid = legacyRuntimeGuid,
                IsActive = true
            }
        };
        _mockQueryService
        ...
        var brandedOptions = options.Where(o =>
            o.PredefinedPlan != null &&
            PowerPlanDefinitions.IsModunePowerPlanGuid(o.PredefinedPlan.Guid)).ToList();
        brandedOptions.Should().ContainSingle();
        brandedOptions[0].SystemPlan.Should().NotBeNull();
        brandedOptions[0].SystemPlan!.Guid.Should().Be(legacyRuntimeGuid);
        brandedOptions[0].IsActive.Should().BeTrue();
        options.Should().NotContain(o =>
            o.PredefinedPlan == null &&
            PowerPlanDefinitions.IsModunePowerPlanName(o.SystemPlan?.Name));
    }
