...
namespace Tweaker.IntegrationTests.Registry;
...
public class WindowsRegistryServiceTests : IDisposable
{
    ...
    public WindowsRegistryServiceTests()
    {
        ...
        _testKeyRoot = $@"Software\TweakerIntegrationTests\{guid}";
    ...
    public void SetValue_String_ReadBackMatches()
    {
        ...
        var testValue = "Hello, Tweaker Integration Tests!";
    ...
    public void Dispose()
    {
        ...
        // Also clean up parent "TweakerIntegrationTests" if empty
        try
        {
            using var parent = Microsoft.Win32.Registry.CurrentUser.OpenSubKey(
                @"Software\TweakerIntegrationTests", writable: false);
            if (parent?.SubKeyCount == 0)
            {
                Microsoft.Win32.Registry.CurrentUser.DeleteSubKeyTree(
                    @"Software\TweakerIntegrationTests", throwOnMissingSubKey: false);
            }
        }
        catch
        {
