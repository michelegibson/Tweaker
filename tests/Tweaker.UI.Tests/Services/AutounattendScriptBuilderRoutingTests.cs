...
/// AutounattendScriptBuilder. Directly exercises BuildTweakermentsScriptAsync and inspects
...
public class AutounattendScriptBuilderRoutingTests
{
    ...
    public async Task PowerShellScript_MarkedUser_LandsInUserPassOnly()
    {
        ...
        var script = await builder.BuildTweakermentsScriptAsync(
    ...
    public async Task RegContents_HkcuMentionedInComment_StillRoutesToSystemPass()
    {
        ...
        var def = new SettingDefinition
        {
            ...
            RegistrySettings = new List<RegistrySetting>
            {
                new()
                {
                    KeyPath = @"HKEY_CLASSES_ROOT\Tweaker.Test",
                    ValueName = "",
                    EnabledValue = ["test"],
                    DisabledValue = [null],
                    DefaultValue = null,
            ...
            RegContents = new List<RegContentSetting>
            {
                new()
                {
                    EnabledContent =
                        ...
                        "[HKEY_CLASSES_ROOT\\Tweaker.Test]\r\n" +
                        "@=\"test\"\r\n",
        ...
        var (system, user) = SplitPasses(script);
        system.Should().Contain(@"HKEY_CLASSES_ROOT\Tweaker.Test").And.Contain("reg import");
        user.Should().NotContain(@"HKEY_CLASSES_ROOT\Tweaker.Test");
    }
    ...
    public async Task RegContents_MixedHiveBlock_ThrowsOnBuild()
    {
        var def = new SettingDefinition
        {
            ...
            RegistrySettings = new List<RegistrySetting>
            {
                new()
                {
                    KeyPath = @"HKEY_LOCAL_MACHINE\SOFTWARE\Tweaker\MixedTest",
                    ValueName = "Flag",
                    EnabledValue = [1],
                    DisabledValue = [0],
                    RecommendedValue = 1,
        ...
        var act = async () => await builder.BuildTweakermentsScriptAsync(
            ConfigWithOptimize("test-feature", item),
            SingleSetting("test-feature", def));
    ...
    public async Task HklmRegistryToggle_LandsInSystemPassOnly()
    {
        var def = new SettingDefinition
        {
            ...
            RegistrySettings = new List<RegistrySetting>
            {
                new()
                {
                    KeyPath = @"HKEY_LOCAL_MACHINE\SOFTWARE\Tweaker\TestKey",
                    ValueName = "Flag",
                    EnabledValue = [1],
                    DisabledValue = [0],
                    RecommendedValue = 1,
        ...
        var (system, user) = SplitPasses(script);
        system.Should().Contain(@"HKLM:\SOFTWARE\Tweaker\TestKey");
        user.Should().NotContain(@"HKLM:\SOFTWARE\Tweaker\TestKey");
    }
