...
///
/// The .tweaker config wire format serializes Selection state as a
...
/// pre-migration .tweaker config file and proves that every Selection entry in it
...
{
    // Test project build output lives at tests/Tweaker.UI.Tests/bin/x64/Debug/net10.0-windows10.0.19041.0/ .
    // From AppContext.BaseDirectory that's 4 ".." hops up to tests/Tweaker.UI.Tests/ , then into Fixtures/.
    private static readonly string FixturePath = Path.Combine(
        AppContext.BaseDirectory, "..", "..", "..", "..", "Fixtures", "pre-migration-sample.tweaker");
    ...
    /// <summary>
    /// Walks a .tweaker JSON document and records every Selection-type entry
    ...
    /// tests agree on what "a Selection entry in a .tweaker config" looks like.
    /// </summary>
