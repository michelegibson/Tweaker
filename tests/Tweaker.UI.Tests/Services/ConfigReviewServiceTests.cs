...
public class ConfigReviewServiceTests : IDisposable
{
    ...
    public void CurrentMode_DefaultsToNormal()
    {
        ...
        service.CurrentMode.Should().Be(TweakerMode.Normal);
    ...
    public async Task EnterReviewModeAsync_SetsCurrentModeToConfigReview()
    {
        ...
        service.CurrentMode.Should().Be(TweakerMode.ConfigReview);
    ...
    public void EnterBuilderMode_SetsBuilderModeAndTarget()
    {
        ...
        service.CurrentMode.Should().Be(TweakerMode.Builder);
    ...
    [Theory]
    [InlineData("Modune Power Plan", "Tweaker Power Plan")]
    [InlineData("Tweaker Power Plan", "Modune Power Plan")]
    public async Task EnterReviewModeAsync_PowerPlanExactBrandAliases_WithDifferentRuntimeGuids_AreEquivalent(
        string currentName,
        string configName)
    {
        const string settingId = "power-plan-brand-alias";
        var config = ArrangePowerPlanReview(
            settingId,
        ...
        service.GetDiffForSetting(settingId).Should().BeNull();
    }

    [Fact]
    public async Task EnterReviewModeAsync_PowerPlanNameContainingLegacyBrand_IsNotTreatedAsAlias()
    {
        const string settingId = "power-plan-non-alias";
        var config = ArrangePowerPlanReview(
            settingId,
            "11111111-2222-3333-4444-555555555555",
            "Tweaker Experimental Plan",
            "aaaaaaaa-bbbb-cccc-dddd-eeeeeeeeeeee",
            "Modune Power Plan");
        ...
        service.GetDiffForSetting(settingId).Should().NotBeNull();
    }

    private UnifiedConfigurationFile ArrangePowerPlanReview(
        string settingId,
        string currentGuid,
        string currentName,
        string configGuid,
        string configName)
    {
        ...
        _mockDiscoveryService
            ...
            .ReturnsAsync(new Dictionary<string, SettingStateResult>
            {
                [settingId] = new SettingStateResult
                {
                    CurrentValue = 0,
                    RawValues = new Dictionary<string, object?>
                    {
                        ["ActivePowerPlanGuid"] = currentGuid,
                        ["ActivePowerPlan"] = currentName
                    }
                }
            });
        _mockComboBoxSetupService
                    ...
                    [FeatureIds.Power] = new ConfigSection
                    {
                        ...
                        Items = new List<ConfigurationItem>
                        {
                            new()
                            {
                                ...
                                Name = "Power Plan",
                                InputType = InputType.Selection,
                                PowerPlanGuid = configGuid,
                                PowerPlanName = configName
                            }
                        }
                    }
                }
