...
public class ThemeServiceTests
{
    ...
    public void CurrentTheme_DefaultsToSystem()
    {
        ...
        service.CurrentTheme.Should().Be(TweakerTheme.System);
    }
    ...
    public void SetTheme_UpdatesCurrentTheme()
    {
        ...
        service.SetTheme(TweakerTheme.DarkNative);

        service.CurrentTheme.Should().Be(TweakerTheme.DarkNative);
    }
    ...
    {
        var service = CreateService();
        TweakerTheme? receivedTheme = null;
        service.ThemeChanged += (sender, theme) => receivedTheme = theme;

        service.SetTheme(TweakerTheme.LightNative);

        receivedTheme.Should().Be(TweakerTheme.LightNative);
    }
    ...
    [Theory]
    [InlineData(TweakerTheme.System)]
    [InlineData(TweakerTheme.LightNative)]
    [InlineData(TweakerTheme.DarkNative)]
    public void SetTheme_SetsCorrectThemeForEachOption(TweakerTheme theme)
    {
        _mockUserPreferences
            .Setup(u => u.SetPreferenceAsync("Theme", It.IsAny<string>()))
            .ReturnsAsync(OperationResult.Succeeded());
    ...
    public void LoadSavedTheme_WithLightNativePreference_SetsLightNative()
    {
        ...
        service.CurrentTheme.Should().Be(TweakerTheme.LightNative);
    }
    ...
    {
        var service = CreateService();
        var receivedThemes = new List<TweakerTheme>();
        service.ThemeChanged += (_, theme) => receivedThemes.Add(theme);

        service.SetTheme(TweakerTheme.LightNative);
        service.SetTheme(TweakerTheme.DarkNative);
        service.SetTheme(TweakerTheme.System);

        receivedThemes.Should().HaveCount(3);
        receivedThemes[0].Should().Be(TweakerTheme.LightNative);
        receivedThemes[1].Should().Be(TweakerTheme.DarkNative);
        receivedThemes[2].Should().Be(TweakerTheme.System);
    }
    ...
    public void SetTheme_WhenSavePreferenceThrows_DoesNotThrow()
    {
        ...
        var act = () => service.SetTheme(TweakerTheme.DarkNative);
